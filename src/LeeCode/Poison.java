package LeeCode;

import java.util.Random;
import java.util.Scanner;

public class Poison {


    public static void main(String[] args) {


        int buckets;// 樣本數
        int N;// 實驗次數

        int needPig;// 需要的受試者
        int posion = new Random().nextInt(1000) + 1;

        Scanner input = new Scanner(System.in);
        while (true)


        {
            System.out.println("輸入實驗樣本數");
            buckets = input.nextInt();
            System.out.println("輸入實驗次數");

            N = input.nextInt();

            while (N <= 0) {
                System.out.println("實驗次數至少須為1,請重新輸入");
                N = input.nextInt();
            }
            int Base=N+1;
            //實驗幾次,一個位置可以驗證N+1個數字,例如實驗1次,2個受驗者可以驗證2*2=4,如果是4次機會,就是5*5


            double temp=Math.log(buckets)/Math.log(Base);
            needPig = (int) Math.ceil(temp);//(Math.ceil((double)(Math.log(buckets) / Math.log(Base))));
            System.out.println("需要" + needPig + "個受試者,每個受試者測試"+Base+"個號碼");
            showDetail(Base, needPig, buckets);

        }

    }

    public static void showDetail(int x, int y, int z) {// x為實驗次數,y為受試者數,z為樣本數

        int poison = new Random().nextInt(z) + 1;

        System.out.println("大獎位置:" + poison);
        StringBuilder sb = new StringBuilder();
        for (int i = y; i > 0; i--) {//有幾個受試者,就有幾個號碼

            for (int j = 0; j < x; j++) {//這邊是最高次方
                //
                int temp = (int) (poison - Math.pow(x, i-1));
                if (temp >= 0) {
                    poison = temp;

                } else {
                    System.out.println("第" + i + "位數字為" + j);
                    sb.append(j);
                    break;
                }

            }

        }

        System.out.println("顯示各輪狀況");
        for(int i=0;i<x-1;i++){//輪數
            System.out.println("第"+(i+1)+"輪數字是"+i);
            for(int j=0;j<y;j++){//人

                if(Character.getNumericValue(sb.charAt(j))==i){
                    System.out.println(j+1+"號豬這一輪掛了");
                }

            }


        }

        System.out.println("中毒號碼為"+sb.toString());

    }




}
